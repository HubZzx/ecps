...
function del(shipAddrId){
	if(confirm("是否确认要删除该地址?")){
		window.location.href = "${path}/user/login/delAddrById.do?shipAddrId="+shipAddrId;
	}
}
...
<div class="w loc">
	...
	<p class="r"><a href="index.jsp" title="首页">首页</a><samp>&gt;</samp><a href="${path }/user/login/toAddr.do" title="用户中心">用户中心</a><samp>&gt;</samp><span class="gray">收货地址</span></p>
</div>
			...
			<div class="ofc">
				...
				<c:forEach items="${addrList }" var="addr">
					...
					>
						...
						<td>${addr.phone }</td>
						<td class="def">
							<a href="${path }/user/login/updateDefault.do?shipAddrId=${addr.shipAddrId}" title="设为默认">设为默认</a>
						</td>
						<td>
							<a href="javascript:void(0);" title="修改" onclick="modify(${addr.shipAddrId})" class="blue">[修改]</a>
							<a href="javascript:void(0);" title="删除" onclick="del(${addr.shipAddrId})" class="blue">[删除]</a>
						</td> 
					</tr>
				</c:forEach>
				...
				<form id="jvForm" action="${path }/user/login/saveOrUpdateAddr.do" method="post">
					...
					<li>
						...
						<span class="word">
							<select name="province" id="province" vld="{required:true}">
								<option value="" selected>省/直辖市</option>
								<option value="吉林">吉林省</option>
								<option value="广西">广西省</option>
							</select>
							<select name="city" id="myCity" vld="{required:true}">
								<option value="" selected>城市</option>
								<option value="长春">长春</option>
								<option value="南宁">南宁</option>
							</select>
							<select  name="district" id="district" vld="{required:true}">
